...
export class AchievementService {
  ...
  async getPaginate(
    ...
    const achievement = await this.microDev
      .send({ cmd: 'achievement_get_paginate' }, { user, limit, page, filtros })
      .toPromise();
    return achievement;
  }
  ...
  async index(user: UserEntity): Promise<AchievementEntity[]> {
    return await this.microDev
      .send({ cmd: 'achievement_index' }, { user })
      .toPromise();
  }

  /* GET A ACHIEVEMENT WITH ID */
  async get(id: number): Promise<AchievementEntity> {
    const achievement = await this.microDev
      .send({ cmd: 'achievement_get' }, { id })
      .toPromise();
    if (!achievement) {
  ...
  /* GET FROM GAME */
  async getFromGame(id: number): Promise<AchievementEntity[]> {
    const game = await this.gameService.get(id);
    if (!game) {
      throw new NotFoundException(`Game not found ${id}`);
    }
    const achievement = await this.microDev
      .send({ cmd: 'achievement_get_from_game' }, { id })
      .toPromise();
    return achievement;
  }
  ...
  async update(
    ...
    return await this.microDev
      .send(
        ...
        { updateAchievementDto, user, achievement, game, file },
      )
      .toPromise();
  }
  ...
  async delete(id: number, user: UserEntity): Promise<any> {
    const achievement = await this.check(id, user);
    return await this.microDev
      .send({ cmd: 'achievement_delete' }, { achievement, user })
      .toPromise();
  }
  ...
  async check(id: number, user: UserEntity): Promise<AchievementEntity> {
    const achievement = await this.microDev
      .send({ cmd: 'achievement_check' }, { id, user })
      .toPromise();
    if (!achievement) {
