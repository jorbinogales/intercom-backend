...
export class EventService {
  ...
  /* STORE */
  async store(createEventDto: CreateEventDto, user: UserEntity): Promise<any> {
    const { game_id } = createEventDto;
    const game = await this.gameService.check(game_id, user);
    return await this.microDev
      .send({ cmd: 'event_store' }, { createEventDto, game, user })
      .toPromise();
  }
  ...
  async indexPaginate(
    ...
    return await this.microDev
      .send({ cmd: 'event_index_paginate' }, { user, page, limit, filtros })
      .toPromise();
  }
  ...
  async index(user: UserEntity): Promise<EventEntity[]> {
    return await this.microDev
      .send({ cmd: 'event_index' }, { user })
      .toPromise();
  }

  /* GET EVENT WITH ID*/
  async get(id: number): Promise<EventEntity> {
    const event = await this.microDev
      .send({ cmd: 'event_get' }, { id })
      .toPromise();
    if (!event) {
  ...
  async getFromGames(game_id: number): Promise<EventEntity[]> {
    ...
    return await this.microDev
      .send({ cmd: 'event_get_from_game' }, { game })
      .toPromise();
  }
  ...
  async update(
    ...
    return await this.microDev
      .send({ cmd: 'event_update' }, { updateEventDto, user, game, event })
      .toPromise();
  }
  ...
  async delete(user: UserEntity, id: number): Promise<any> {
    const event = await this.check(id, user);
    return await this.microDev
      .send({ cmd: 'event_delete' }, { event, user })
      .toPromise();
  }
}
